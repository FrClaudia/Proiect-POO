...
public:
	...
	friend void meniu(TABELA*& tab, int& nr_tab);
	friend void comanda_utilizator(const char* com, TABELA*& tab_utilizator, int& nr_tabele, int& okay);
};
...
ostream& operator<<(ostream& out, TABELA tb)
{
	...
	out << tb.nr_total_coloane << endl << endl;
	int te = 0; //contoare
	int in = 0;
	int fl = 0;
